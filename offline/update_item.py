...
class UpdateItem(SparkSessionBase):
        ...
        def func(partition):
            ...
            for row in partition:
                ...
                for word_index, tfidf in result:
                    yield row.id, row.title, int(word_index), round(float(tfidf), 4)

        _keywordsByTFIDF = tfidf_result.rdd.mapPartitions(func).toDF(["id", "index", "query", "tfidf"])
    ...
    def merge_item_data(self, sentence_df):
        """
        ...
        # 工具与业务隔离
        _keywordsByTFIDF = UpdateItem.compute_keywords_tfidf_topk(words_df, cv_model, idf_model)
...
if __name__ == '__main__':
    ...
    if sentence_df.rdd.collect():
        rank, idf = ua.merge_item_data(sentence_df)
