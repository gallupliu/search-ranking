...
CENSUS_CONFIG = {
    'columns':  ['age','workclass','fnlwgt','education','education_num','marital_status','occupation','relationship','race','gender','capital_gain','capital_loss','hours_per_week','native_country','income_bracket','text'],
...
def build_census_feat_columns(emb_dim=8):
    def _get_numeric_feat_range():
        train = pd.read_csv('./data/raw/adult/adult.data',header=None,names=CENSUS_CONFIG['columns'])[CENSUS_CONFIG['deep_bucket_emb_cols']]
        test = pd.read_csv('./data/raw/adult/adult.test',header=None,names=CENSUS_CONFIG['columns'])[CENSUS_CONFIG['deep_bucket_emb_cols']]
