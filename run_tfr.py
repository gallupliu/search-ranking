...
def print_tfrecord(filename):
    filenames = [filename]
    raw_dataset = tf.data.TFRecordDataset(filenames)
    # Create a description of the features.
    feature_description = {
        'relevant':  tf.io.FixedLenFeature([], dtype=tf.string),
        'query_tokens': tf.io.FixedLenFeature([], dtype=tf.string),
        'document_tokens': tf.io.FixedLenFeature([], dtype=tf.string)
    }
...
def write_recodes(dataes, out_file_name):
...
def main(_):
    print_tfrecord(FLAGS.train_input_pattern)
