...
class Que2Search(Model):
    ...
    def __init__(self, config,
        ...
        self.bert_text_cols = config['bert_text_cols']
        ...
        self.d_model=config['user_d_model']
        ...
        self.user_encoder = Encoder(num_layers=config['user_num_layers'], d_model=config['user_d_model'],
                                    num_heads=config['user_num_heads'],
                                    dff=config['dff'], input_vocab_size=config['bert_vocab_size'],
                                    maximum_position_encoding=config['maximum_position_encoding'])

        self.item_encoder = Encoder(num_layers=config['item_num_layers'], d_model=config['item_d_model'],
                                    num_heads=config['item_num_heads'],
                                    dff=config['dff'], input_vocab_size=config['bert_vocab_size'],
                                    maximum_position_encoding=config['maximum_position_encoding'])
        ...
        for feat in self.user_cols:
            ...
            elif feat['name'] in self.bert_text_cols:
                self.user_embed_layers['embed_' + str(feat['name'])] = self.user_encoder
            else:
            ...
            for feat in self.item_cols:
                ...
                if feat['name'] in self.text_cols:
                    ...
                    print(feat['name'])
                    self.item_embed_layers['embed_' + str(feat['name'])] = self.item_encoder
    ...
    def call(self, inputs, training=False, mask=None):
        ...
        for k, v in user_inputs.items():
            ...
            if k not in self.bert_text_cols:
        ...
        user_bert_enc = self.user_encoder(user_bert_dict['keyword_input_ids'],True,user_bert_dict['keyword_attention_mask'])
        user_bert_enc = self.pooling_embedding(user_bert_enc)
        user_bert_enc = tf.reshape(user_bert_enc, [-1, 1, self.d_model])
        #需要提取cls对应向量或者对所有向量pooling
        user_sparse_embed = tf.concat(user_embedding+[user_bert_enc], axis=-1)
        ...
        item_bert_enc = self.item_encoder(item_bert_dict['item_input_ids'],True,item_bert_dict['item_attention_mask'])
        #需要提取cls对应向量或者对所有向量pooling
        item_bert_enc = self.pooling_embedding(item_bert_enc)
        item_bert_enc = tf.reshape(item_bert_enc, [-1, 1, self.d_model])
        print('embed_user:{0},item:{1}'.format(item_embedding, item_embedding))
        item_sparse_embed = tf.concat(item_embedding+[item_bert_enc], axis=-1)
